...
const SignUp = () => {
  ...
  const inputVerifyPassword = useCallback((event) => {
  ...
  return (
    <div className="c-section-container">
      ...
      />
      <TextInput
        fullWidth={true} label={"Verify Password"} multiline={false} required={true}
        rows={1} value={verifyPassword} type={"password"} onChange={inputVerifyPassword}
      />
      <Spacer />
      <div className="center">
        <PrimaryButton
        label="Register" 
        onClick={() => dispatch(signUp(username, email, password, verifyPassword))} 
        />
      </div>
    </div>
  )
